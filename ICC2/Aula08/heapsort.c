/*
Heap sort é uma técnica de ordenação baseada em comparação baseada na Estrutura de Dados de Heap Binário. 
Pode ser visto como uma otimização sobre o tipo de seleção, onde primeiro encontramos o elemento max (ou min) e
o trocamos pelo último (ou primeiro). Repetimos o mesmo processo para os elementos restantes.
No Heap Sort, usamos o Heap binário para que possamos encontrar e mover rapidamente o elemento max (In O(Log n)
em vez de O(n)) e, portanto, atingir a complexidade de tempo O(n Log n).
